...
public class DepthMapView extends View {
    ...
    private List<DepthData> mBuyData = new ArrayList<>();
    private List<DepthData> mSellData = new ArrayList<>();

    //    价格显示精度限制
    public int mPriceLimit = 4;
    private int mVolumeLimit = 0;

    private HashMap<Integer, DepthData> mMapX;
        ...
        if (typedArray != null) {
            try {
                ...
                if (!isBig) {
                    ...
                    mTextPaint.setTextSize(typedArray.getDimension(R.styleable.DepthMapView_depth_mv_text_size, ResourceUtil.getDimension(getContext(), R.dimen.depth_text_small_size)));
                } else {
    ...
    public void setData(List<DepthData> buyData, List<DepthData> sellData) {
        mMaxVolume = 0;
        if (buyData.size() > 0) {
            mBuyData.clear();
...
//                    mIsLongPress = true;
    ...
    public class comparePrice implements Comparator<DepthData> {
        @Override
